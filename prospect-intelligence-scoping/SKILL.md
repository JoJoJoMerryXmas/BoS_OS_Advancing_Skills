...
- **FIT**, is this the right kind of company? Mostly *public and discoverable* so can be scored automatically across the whole market.
...
|---|---|---|---|
| **Attribute** | A stable fact about the company | headcount, industry, tech stack, location, firm size | green, look it up |
| **Event / trigger** | Something that *happened*, time-bound | data breach, funding round, exec hire, layoffs, new office, product launch | green, look it up (news / registries / disclosures / job boards) |
| **Judgement** | A subjective call, no public field states it | "AI leader", "innovative", "security-conscious", "serious about X" | yellow, needs a proxy or a manual tag |
| **Private fact** | Real, but only knowable from inside | "uses competitor X", "has budget", "unhappy with vendor" | red, first-party/intent overlay, or drop |
...
- Green, **Discoverable**, a public/accessible source plainly reveals this (headcount from company-data providers; hiring/breach/funding from news & job boards & registries; a person's role from LinkedIn).
- Yellow, **Proxy needed**, not directly public, but a discoverable signal correlates with it. Name the proxy and how good it is.
- Red, **Not discoverable**, only knowable internally. Demote to a manual/intent overlay, or drop. Say so plainly.
...
|------|----------------------|------|---------------|---------|---------------------|--------------|
```
Green/yellow/red per criterion; Type in {attribute, event, judgement, private}; Coverage from the Phase-4 sample. This is the honest heart of the output, it tells the operator what will and won't work, *and how often*, before they build.
...
- **Known gaps & proxies**, every red and yellow, with the fallback chosen.
...
**v1.0.1 (2026-08-12)** Packaging fix only, no content change. The originally-released `.skill` package had `SKILL.md` nested one level too deep (`prospect-intelligence-scoping/SKILL.md` inside the zip, instead of at the zip root), which breaks Cowork's skill uploader. Repackaged correctly; nothing in this file changed except this version line.
